...
class Rabisco(Figura):
    ...
    def __init__(self, pontos, cor_borda):
        super().__init__(cor_borda)
        self.pontos = pontos[:]  # cópia da lista
    
    def desenhar(self, canvas):
        # O Tkinter precisa de no mínimo 4 coordenadas (x1, y1, x2, y2) para traçar uma linha
...
class PoligonoRegular(Figura):
    """Representa um polígono regular (triângulo, quadrado, pentágono...)."""
    ...
    def _calcular_pontos(self):
        pontos = []
        # Evita divisão por zero se num_lados for inválido
